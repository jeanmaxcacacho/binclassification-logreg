...
In this section logistic regression is run three times: (i) without resampling, (ii) resampling with SMOTE, and (iii) resampling with RUS. The target data distribution is heavily imbalanced, making the dataset a good candidate for analyzing performance contributions/effects of resampling techniques.
...
The previous subsection showed room for improvement with the NPV of the model. One of the main challenges in classification tasks is deciding how to handle class imbalances, one of the ways this is adressed is through resampling. Resampling, makes it so that the proportions in the training data is more balanced, giving the model more exposure to both targets.

The resampling approaches used in this project are: **(i) Oversampling with SMOTE, and (ii) Undersampling with RUS**. 

### Train & Test Set Creation for SMOTE
```{r}
for_SMOTE <- copy(raw_df)

set.seed("123")

split_SMOTE <- caTools::sample.split(for_SMOTE$diabetes, SplitRatio = 0.8)

SMOTE_train_data <- subset(for_SMOTE, split_SMOTE == TRUE)
SMOTE_test_data <- subset(for_SMOTE, split_SMOTE == FALSE)

print(SMOTE_train_data)
```
### SMOTE Resampling

SMOTE works by synthetically generating more observations of the minority class. For each minority instance, it uses KNN to generate synthetic observations. The process stops when the desired class balance is achieved, usually this is 50:50. The class balance considered for the amount of new instances created per minority instance and majority instances kept is the distribution in the training data.

```{r}
SMOTE_targcounts <- table(SMOTE_train_data$diabetes)

barplot(
  SMOTE_targcounts,
  names.arg = c("Non-diabetic", "Diabetic"),
  col = "steelblue",
  main = "Target Variable Distribution in SMOTE Train Set",
  xlab = "Classification",
  ylab = "Observation Frequency")
```
...
targ_ratio
```
The ratio of majority to minority class is about 8:1. From this we can resample the train set accordingly. SMOTE implementation used in this project is from the `smotefamily` package.
```{r}
# separate features from label
features <- names(SMOTE_train_data)[names(SMOTE_train_data) != "diabetes"]
X <- SMOTE_train_data[, ..features]

y <- SMOTE_train_data$diabetes

# str(X)
# str(y)
SMOTEd <- smotefamily::SMOTE(X, y, K = 3, dup_size = 7)
```
